<template>
  <div class="sys-region-container">
          ...
          <el-tabs tab-position="left" v-model="tabPosition">
            ...
            <el-tab-pane label="数据源设置" name="1" style="height:600px;overflow-y: auto;">
              <el-form-item label="DataSet" prop="dataSetId" :rules="[{ required: true, message: '请选择dataSet', trigger: 'blur' }]">
                <el-select filterable remote :loading="state.dataSetLoading" :remote-method="(value:string)=>remoteMethod(value,'0')" v-model="state.tableModel.dataSetId" @change="(value:string)=>dataSetSelect(value,'0')">
                ...
                <el-card class="box-card" v-for="(row,index) in state.tableModel.rowColorList" :key="index">
                  <template #header>
                    <div class="card-header">
                      <span>{{ row.name }}</span>
                      <el-button class="button" @click="()=>{state.tableModel.rowColorList.splice(index,1)}">删除</el-button>
                    </div>
                  </template>
                  <div>当：</div>
    ...
    <el-dialog v-model="state.isGuDing">
      ...
      <div class="sidel" style="height:400px; overflow-y: auto;">
        <div v-for="(g,index) in state.guDingList" :key="index" style="line-height: 50px;;">
          value:<el-input v-model="g.value" style="width: 30%;" />
          label:<el-input v-model="g.label" style="width: 30%;" />
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="()=>{state.isGuDing=false}">取 消</el-button>
    ...
    <el-dialog v-model="state.isZhiDing">
      ...
      <div class="sidel" style="height:400px; overflow-y: auto;">
        <div>
          <el-select filterable remote :loading="state.dataSetLoading" :remote-method="(value:string)=>remoteMethod(value,'1')" v-model="state.zhiDingObj.dataSetId" @change="(value:string)=>dataSetSelect(value,'1')">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="()=>{state.isZhiDing=false}">取 消</el-button>
